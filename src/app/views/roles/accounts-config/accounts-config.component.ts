...
@Component({
  ...
  styleUrls: ['./accounts-config.component.scss']
})
export class AccountsConfigComponent implements OnInit {
  ...
  data:any =[
    {
      names:'Accounts Menu',
      accountsMenu:[
        { names:'NA',value:'NA',checked:'NA',arrnames:'ACCOUNTS_MENU',module:'ACCOUNTS'},
        { names:'NA',value:'NA',checked:'NA',arrnames:'ACCOUNTS_MENU',module:'ACCOUNTS'},
        { names:'VIEW',value:'VIEW',checked:false,arrnames:'ACCOUNTS_MENU',module:'ACCOUNTS'},
        { names:'NA',value:'NA',checked:'NA',arrnames:'ACCOUNTS_MENU',module:'ACCOUNTS'},
      ]
    },
    {
      names:'Subcription Plan',
      subscriptionPlan:[
        { names:'NA',value:'NA',checked:'NA',arrnames:'SUBCRIPTION_PLAN',module:'ACCOUNTS'},
        { names:'NA',value:'NA',checked:'NA',arrnames:'SUBCRIPTION_PLAN',module:'ACCOUNTS'},
        { names:'VIEW',value:'VIEW',checked:false,arrnames:'SUBCRIPTION_PLAN',module:'ACCOUNTS'},
        { names:'NA',value:'NA',checked:'NA',arrnames:'SUBCRIPTION_PLAN',module:'ACCOUNTS'},
      ]
    }
  ]
  ...
  this.api.getUserRoleById(`id=${this.id}&page_number=1&data_per_page=10&pagination=TRUE&organization_id=${this.orgId}`).subscribe(res=>{
    if (res) {
      ...
      this.permission.forEach(item => {
              ...
              this.data.forEach(dataItem => {
                Object.keys(dataItem).forEach(dataKey => {
                  if (Array.isArray(dataItem[dataKey])) {
                    ...
                    // if (option.names === 'NA') {
                    //   allNonNAOptionsChecked = true;
                    //  // option.checked = true; // Set 'NA' options as checked (true)
                    // } else  {
                    //   allNonNAOptionsChecked = false; // If any non-'NA' option is not checked, set the flag to false
                    // }
                    });
                  }
                });
       ...
       //this.checkAll = allNonNAOptionsChecked
  ...
  removePermissionsIfExists(selectedOptions) {
    return this.permission.map((item) => {
      const permissionIndex = item.module_name.indexOf("ACCOUNTS");
      if (permissionIndex !== -1) {
        item.module_name.splice(permissionIndex, 1);
        item.permissions.splice(permissionIndex, 1);
        // Add ACCOUNTS to module_name array at index 0
        item.module_name.unshift("ACCOUNTS");
