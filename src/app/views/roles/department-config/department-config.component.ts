...
export class DepartmentConfigComponent implements OnInit {
  data:any = [
    {
      names:'Department',
      department:[
        {names:'CREATE', value:'CREATE',checked:false,arrnames:'DEPARTMENT',module:'DEPARTMENT'},
        {names:'UPDATE', value:'UPDATE',checked:false,arrnames:'DEPARTMENT',module:'DEPARTMENT'},
        {names:'VIEW', value:'VIEW',checked:false,arrnames:'DEPARTMENT',module:'DEPARTMENT'},
        {names:'DELETE', value:'DELETE',checked:false,arrnames:'DEPARTMENT',module:'DEPARTMENT'},
      ]
    }
  ]
  userRole: string;
  ...
  removePermissionsIfExists(selectedOptions) {
    return this.permission.map((item) => {
      const permissionIndex = item.module_name.indexOf("DEPARTMENT");
      if (permissionIndex !== -1) {
        item.module_name.splice(permissionIndex, 1);
        item.permissions.splice(permissionIndex, 1);
        // Add DEPARTMENT to module_name array at index 0
        item.module_name.unshift("DEPARTMENT");
