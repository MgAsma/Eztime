...
export class ReviewConfigComponent implements OnInit {
  data:any =[
    {
      names:'Review',
      review:[
       {names:'VIEW',value:'VIEW',checked:false,arrnames:'REVIEW',module:'REVIEW'},
       {names:'APPROVE',value:'APPROVE',checked:false,arrnames:'REVIEW',module:'REVIEW'},
       {names:'REJECT',value:'REJECT',checked:false,arrnames:'REVIEW',module:'REVIEW'}
      ]
    }
  ]
  userRole: string;
  ...
  removePermissionsIfExists(selectedOptions) {
    return this.permission.map((item) => {
      const permissionIndex = item.module_name.indexOf("REVIEW");
      if (permissionIndex !== -1) {
        item.module_name.splice(permissionIndex, 1);
        item.permissions.splice(permissionIndex, 1);
        // Add REVIEW to module_name array at index 0
        item.module_name.unshift("REVIEW");
